...
Usage:
    python3 -m pipelines.content_retriever.run --config configs/rednote_example.yaml
    python3 -m pipelines.content_retriever.run --config configs/rednote_example.yaml --max-posts 5
    python3 -m pipelines.content_retriever.run --config configs/rednote_example.yaml --summarize-backend ollama --summarize-model qwen2.5:7b
...
# Ensure pipelines root is on sys.path when invoked directly
_HERE = Path(__file__).resolve().parent
_PIPELINES_ROOT = _HERE.parent.parent          # …/VoidDraft
...
from pipelines.content_retriever.graph import build_graph
